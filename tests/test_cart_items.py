...
def _setup(client) -> tuple[int, int, int]:
    ...
    uid = client.post(
        "/api/v1/users/",
        json={"email": "shopper@example.com", "password": "secret", "name": "Shopper"},
    ).json()["id"]
    pid = client.post("/api/v1/products/", json={"name": "Item"}).json()["id"]
    vid = client.post(
        f"/api/v1/products/{pid}/variants",
        json={"sku": "SHP-1", "price": 25.0, "inventory_qty": 100},
    ).json()["id"]
    return uid, pid, vid
...
def test_add_item_to_cart_returns_201(client: TestClient):
    ...
    r = client.post(
        f"/api/v1/carts/{c['id']}/items", json={"product_variant_id": vid, "quantity": 2}
    )
...
def test_update_item_quantity_changes_line_total(client: TestClient):
    ...
    c = client.post("/api/v1/carts/", json={"user_id": uid}).json()
    item = client.post(
        f"/api/v1/carts/{c['id']}/items", json={"product_variant_id": vid, "quantity": 1}
    ).json()["items"][0]
    r = client.patch(
...
def test_add_multiple_items_to_cart(client: TestClient):
    ...
    uid = client.post(
        "/api/v1/users/", json={"email": "multi@example.com", "password": "secret", "name": "M"}
    ).json()["id"]
    p1 = client.post("/api/v1/products/", json={"name": "P1"}).json()["id"]
    p2 = client.post("/api/v1/products/", json={"name": "P2"}).json()["id"]
    v1 = client.post(
        f"/api/v1/products/{p1}/variants", json={"sku": "M-1", "price": 10.0, "inventory_qty": 50}
    ).json()["id"]
    v2 = client.post(
        f"/api/v1/products/{p2}/variants", json={"sku": "M-2", "price": 20.0, "inventory_qty": 50}
    ).json()["id"]
    c = client.post("/api/v1/carts/", json={"user_id": uid}).json()
    client.post(f"/api/v1/carts/{c['id']}/items", json={"product_variant_id": v1, "quantity": 1})
    r = client.post(
        f"/api/v1/carts/{c['id']}/items", json={"product_variant_id": v2, "quantity": 1}
    )
    assert r.status_code == 201
...
def test_cart_total_applies_discount(client: TestClient):
    ...
    uid = client.post(
        "/api/v1/users/", json={"email": "disc@example.com", "password": "secret", "name": "D"}
    ).json()["id"]
    pid = client.post("/api/v1/products/", json={"name": "DiscItem"}).json()["id"]
    vid = client.post(
        f"/api/v1/products/{pid}/variants",
        json={"sku": "DI-1", "price": 100.0, "inventory_qty": 10},
    ).json()["id"]
...
def test_add_duplicate_item_returns_409(client: TestClient):
    ...
    uid, _, vid = _setup(client)
    c = client.post("/api/v1/carts/", json={"user_id": uid}).json()
    client.post(f"/api/v1/carts/{c['id']}/items", json={"product_variant_id": vid, "quantity": 1})
    r = client.post(
        f"/api/v1/carts/{c['id']}/items", json={"product_variant_id": vid, "quantity": 1}
    )
...
def test_add_item_with_quantity_exceeding_inventory_returns_422(client: TestClient):
    uid = client.post(
        "/api/v1/users/", json={"email": "inv@example.com", "password": "secret", "name": "I"}
    ).json()["id"]
    pid = client.post("/api/v1/products/", json={"name": "LowStock"}).json()["id"]
    vid = client.post(
        f"/api/v1/products/{pid}/variants", json={"sku": "LOW-1", "price": 50.0, "inventory_qty": 2}
    ).json()["id"]
    c = client.post("/api/v1/carts/", json={"user_id": uid}).json()
    r = client.post(
        f"/api/v1/carts/{c['id']}/items", json={"product_variant_id": vid, "quantity": 100}
    )
...
def test_add_item_with_nonexistent_variant_returns_404(client: TestClient):
    uid = client.post(
        "/api/v1/users/", json={"email": "nf@example.com", "password": "secret", "name": "N"}
    ).json()["id"]
    c = client.post("/api/v1/carts/", json={"user_id": uid}).json()
    r = client.post(
        f"/api/v1/carts/{c['id']}/items", json={"product_variant_id": 99999, "quantity": 1}
    )
...
def test_add_item_with_zero_quantity_returns_422(client: TestClient):
    ...
    c = client.post("/api/v1/carts/", json={"user_id": uid}).json()
    r = client.post(
        f"/api/v1/carts/{c['id']}/items", json={"product_variant_id": vid, "quantity": 0}
    )
    assert r.status_code == 422
