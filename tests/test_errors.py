...
def test_create_user_missing_required_field_returns_422(client: TestClient):
    ...
    r = client.post("/api/v1/users/", json={"email": "noName@example.com", "password": "secret"})
    assert r.status_code == 422


def test_create_user_invalid_email_type_returns_422(client: TestClient):
    """Email must be a string."""
    r = client.post("/api/v1/users/", json={"email": 12345, "password": "secret", "name": "Bad"})
    assert r.status_code == 422
...
def test_update_item_with_negative_quantity_returns_422(client: TestClient):
    ...
    uid = client.post(
        "/api/v1/users/", json={"email": "neg@example.com", "password": "secret", "name": "N"}
    ).json()["id"]
    pid = client.post("/api/v1/products/", json={"name": "Neg"}).json()["id"]
    vid = client.post(
        f"/api/v1/products/{pid}/variants", json={"sku": "NEG-1", "price": 1.0, "inventory_qty": 10}
    ).json()["id"]
...
def test_get_deleted_user_returns_404(client: TestClient):
    ...
    u = client.post(
        "/api/v1/users/", json={"email": "del@example.com", "password": "secret", "name": "D"}
    ).json()
...
def test_update_user_to_existing_email_returns_409(client: TestClient):
    ...
    client.post(
        "/api/v1/users/", json={"email": "alpha@example.com", "password": "secret", "name": "A"}
    )
    b = client.post(
        "/api/v1/users/", json={"email": "beta@example.com", "password": "secret", "name": "B"}
    ).json()
...
def test_update_variant_to_existing_sku_returns_409(client: TestClient):
    ...
    client.post(
        f"/api/v1/products/{pid}/variants", json={"sku": "S-A", "price": 1.0, "inventory_qty": 1}
    )
    v2 = client.post(
        f"/api/v1/products/{pid}/variants", json={"sku": "S-B", "price": 1.0, "inventory_qty": 1}
    ).json()
    r = client.patch(f"/api/v1/products/variants/{v2['id']}", json={"sku": "S-A"})
    assert r.status_code == 409
...
def test_delete_user_with_existing_cart_returns_409(client: TestClient):
    ...
    u = client.post(
        "/api/v1/users/", json={"email": "fk@example.com", "password": "secret", "name": "FK"}
    ).json()
