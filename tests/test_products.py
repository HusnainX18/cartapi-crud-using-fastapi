...
def test_create_product_returns_201(client: TestClient):
    r = client.post("/api/v1/products/", json={
        "name": "Laptop", "description": "16GB RAM", "brand": "Acme"
    })
...
def test_get_product_includes_variants(client: TestClient):
    ...
    client.post(f"/api/v1/products/{p['id']}/variants", json={
        "sku": "KB-001", "price": 49.99, "inventory_qty": 5
    })
...
def test_delete_product_cascades_to_variants(client: TestClient):
    ...
    v = client.post(f"/api/v1/products/{p['id']}/variants", json={
        "sku": "DEL-001", "price": 1.0, "inventory_qty": 1
    }).json()
...
def test_create_variant_for_nonexistent_product_returns_404(client: TestClient):
    r = client.post("/api/v1/products/99999/variants", json={
        "sku": "X-1", "price": 1.0, "inventory_qty": 0
    })
    assert r.status_code == 404
...
def test_create_variant_with_negative_price_returns_422(client: TestClient):
    ...
    r = client.post(f"/api/v1/products/{p['id']}/variants", json={
        "sku": "BAD-1", "price": -5.0, "inventory_qty": 1
    })
    assert r.status_code == 422
...
def test_create_variant_with_zero_inventory_is_allowed(client: TestClient):
    ...
    r = client.post(f"/api/v1/products/{p['id']}/variants", json={
        "sku": "PO-1", "price": 99.0, "inventory_qty": 0
    })
    assert r.status_code == 201
