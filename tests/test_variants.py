...
def test_create_variant_attaches_to_product(client: TestClient):
    ...
    r = client.post(
        ...
        json={"sku": "P-001", "price": 199.0, "inventory_qty": 50},
    )
...
def test_get_variant_by_id(client: TestClient):
    pid = _make_product(client)
    v = client.post(
        f"/api/v1/products/{pid}/variants", json={"sku": "G-1", "price": 10.0, "inventory_qty": 5}
    ).json()
...
def test_update_variant_changes_price_and_qty(client: TestClient):
    pid = _make_product(client)
    v = client.post(
        f"/api/v1/products/{pid}/variants", json={"sku": "U-1", "price": 10.0, "inventory_qty": 5}
    ).json()
    r = client.patch(
        f"/api/v1/products/variants/{v['id']}", json={"price": 12.5, "inventory_qty": 8}
    )
    assert r.status_code == 200
    body = r.json()
...
def test_delete_variant_returns_200(client: TestClient):
    pid = _make_product(client)
    v = client.post(
        f"/api/v1/products/{pid}/variants", json={"sku": "D-1", "price": 1.0, "inventory_qty": 1}
    ).json()
...
def test_create_variant_with_duplicate_sku_returns_409(client: TestClient):
    ...
    pid = _make_product(client)
    r1 = client.post(
        f"/api/v1/products/{pid}/variants", json={"sku": "DUP-1", "price": 1.0, "inventory_qty": 1}
    )
    assert r1.status_code == 201
    r2 = client.post(
        f"/api/v1/products/{pid}/variants", json={"sku": "DUP-1", "price": 2.0, "inventory_qty": 2}
    )
